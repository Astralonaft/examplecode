...
try {
	...
	if (!empty($url[1]))
	{
		...
		$controller = new user($Request, $ROOT_REST);
		...
		$CSS .= '<link rel="stylesheet" href="' . $HTTP . $HTTP_HOST . 'assets/css/toast/toast.css">';
		# 	Подключим только для этой страницы
		$JS = '<script src="' . $HTTP . $HTTP_HOST . 'assets/js/vue.js"></script>';
		$JS .= '<script src="' . $HTTP . $HTTP_HOST . 'assets/js/vue_components/vue_component_select.js?v=1"></script>';
		$JS .= '<script src="' . $HTTP . $HTTP_HOST . 'assets/js/vue_directives/vue_input_validate.js?v=' . time() . '"></script>';
		$JS .= '<script src="' . $HTTP . $HTTP_HOST . 'assets/js/toast/toast.js"></script>';
		...
		#	Свой js
		$JSfoot .= '<script src="' . $HTTP . $HTTP_HOST . 'assets/js/user/user.js?v=' . time() . '"></script>'; // ?v=1

		#	TODO AJAX check
		if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
		{
			echo $controller->AjaxHandler();
			die();
		}
		#   TODO html head
